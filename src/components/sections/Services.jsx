...
const ACTION_ICONS = { Rocket, Phone }
 ...
 * reference. Only ever rendered here, so it lives beside its one caller.
 */
function ActionCards({ reduce }) {
  return (
    <Stagger className="mt-3 flex gap-2.5">
      {actionCards.map((card) => {
        const Icon = ACTION_ICONS[card.icon] ?? Rocket
        return (
